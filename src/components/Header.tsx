...
// Prop para controlar la visibilidad/animación del título
...
const Header: React.FC<HeaderProps> = ({ showHeaderTitle = true }) => {
    ...
    const fetchUserRole = async () => {
      try {
        ...
        if (!snap.empty) {
          ...
          // Si isActive falta, lo consideramos true (compatibilidad)
  ...
  // Desktop: abre/cierra dropdown de usuario
  ...
  const renderUserMenuOptions = () => {
    const base = [
      { href: '/profile', icon: <User size={16} />, label: 'Perfil', color: 'hover:text-yellow-400' },
      { href: '/settings', icon: <Settings size={16} />, label: 'Configuración', color: 'hover:text-yellow-400' }
    ];

    const extras: { href: string; icon: JSX.Element; label: string; color: string }[] = [];

    if (userRole === 'super_admin') {
      extras.push({ href: '/AdminDashboard', icon: <Shield size={16} />, label: 'Panel Admin', color: 'hover:text-green-400' });
    } else if (userRole === 'admin') {
      extras.push({ href: '/AdminDashboard', icon: <UserCheck size={16} />, label: 'Panel Admin', color: 'hover:text-blue-400' });
    } else if (userRole === 'collaborator') {
      // ⬇️ El enlace correcto al catálogo de colaborador
      extras.push({ href: '/collaborations', icon: <UserCheck size={16} />, label: 'Herramientas', color: 'hover:text-purple-400' });
    }
  ...
  return (
    <header
      className={`sticky top-0 w-full z-50 transition-all duration-300 ${
        isScrolled ? 'bg-slate-900/95 backdrop-blur-sm shadow-xl' : 'bg-slate-900'
      }`}
    >
          ...
          <div className="relative">
            <div className="relative bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-full px-4 md:px-6 py-3 shadow-lg flex items-center gap-2 md:gap-4">
                  ...
                  <div className="relative">
                    ...
                    <div
                      className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-yellow-400 transition-all duration-1000 ease-in-out ${
                        showHeaderTitle ? 'rounded-full w-0 h-0 opacity-0 scale-0' : 'rounded-full w-3 h-3 opacity-100 scale-100'
                      }`}
                    >
                      <div className={`w-full h-full bg-yellow-400 rounded-full ${!showHeaderTitle ? 'animate-pulse' : ''}`} />
                    </div>
                    {/* Texto Nova Store */}
                    <button
                      onClick={goHome}
                      className={`relative text-lg font-bold text-yellow-400 hover:text-yellow-300 whitespace-nowrap transition-all duration-900 ease-out ${
                        showHeaderTitle ? 'opacity-100 scale-100 filter-none tracking-normal' : 'opacity-0 scale-50 blur-md tracking-widest'
                      }`}
                      style={{
                        textShadow: showHeaderTitle ? 'none' : '0 0 10px rgba(250, 204, 21, 0.5)',
                        transform: showHeaderTitle ? 'translateY(0px) rotateX(0deg)' : 'translateY(2px) rotateX(15deg)'
                      }}
                    >
                      <span className={`inline-block transition-all duration-1000 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>N</span>
                      <span className={`inline-block transition-all duration-1000 delay-75 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>o</span>
                      <span className={`inline-block transition-all duration-1000 delay-150 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>v</span>
                      <span className={`inline-block transition-all duration-1000 delay-225 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>a</span>
                      <span className={`inline-block transition-all duration-1000 delay-300 mx-1 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>{' '}</span>
                      <span className={`inline-block transition-all duration-1000 delay-375 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>S</span>
                      <span className={`inline-block transition-all duration-1000 delay-450 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>t</span>
                      <span className={`inline-block transition-all duration-1000 delay-525 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>o</span>
                      <span className={`inline-block transition-all duration-1000 delay-600 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>r</span>
                      <span className={`inline-block transition-all duration-1000 delay-675 ${showHeaderTitle ? 'transform-none' : 'scale-75'}`}>e</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1">
                ...
                {/* Hamburguesa desktop -> Dropdown usuario */}
                <button
            ...
            {showUserMenu && (
              <div
                className="absolute right-0 top-full mt-4 w-64 bg-slate-800/95 backdrop-blur-sm shadow-xl rounded-2xl overflow-hidden z-50 border border-slate-700/50"
      ...
      {isMobileMenuOpen && (
        <div
          className="md:hidden bg-slate-800/95 backdrop-blur-sm border-t border-slate-700/50 shadow-xl"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="p-6">
            {user ? (
