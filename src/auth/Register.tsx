...
const Register: React.FC<RegisterProps> = ({ onSuccess }) => {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3 animate-scale-in">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`classic-input w-full pl-10 pr-10 py-2.5 text-sm placeholder-[#8A8A8A] dark:placeholder-[#666] ${
            email ? (emailValid ? '!border-accent-success' : '!border-accent-error') : ''
          }`}
          required
        />
        {email && (
          <div className="absolute right-3 top-1/2 -translate-y-1/2 z-10">
            {emailValid ? <CheckCircle size={16} className="text-accent-success" /> : <XCircle size={16} className="text-accent-error" />}
          </div>
        )}
      </div>
      ...
      {/* requisitos */}
      {pw && (
        <div className="bg-white dark:bg-black border-4 border-black dark:border-white p-3 space-y-2 shadow-classic-md">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs font-medium relative z-10">
            <div className={`flex items-center gap-1.5 ${reqs.length ? 'text-accent-success' : 'text-accent-error'}`}>
              {reqs.length ? <CheckCircle size={12} /> : <XCircle size={12} />} 8+ caracteres
            </div>
            <div className={`flex items-center gap-1.5 ${reqs.upper ? 'text-accent-success' : 'text-accent-error'}`}>
              {reqs.upper ? <CheckCircle size={12} /> : <XCircle size={12} />} Mayúscula
            </div>
            <div className={`flex items-center gap-1.5 ${reqs.lower ? 'text-accent-success' : 'text-accent-error'}`}>
              {reqs.lower ? <CheckCircle size={12} /> : <XCircle size={12} />} Minúscula
            </div>
            <div className={`flex items-center gap-1.5 ${reqs.num ? 'text-accent-success' : 'text-accent-error'}`}>
              {reqs.num ? <CheckCircle size={12} /> : <XCircle size={12} />} Número
            </div>
            <div className={`flex items-center gap-1.5 col-span-2 ${reqs.special ? 'text-accent-success' : 'text-accent-error'}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`classic-input w-full pl-10 pr-10 py-2.5 text-sm placeholder-[#8A8A8A] dark:placeholder-[#666] ${
            pw2 ? (match ? '!border-accent-success' : '!border-accent-error') : ''
          }`}
          required
          minLength={8}
        />
      ...
      <button
        ...
        className={`w-full border-4 border-black dark:border-white text-sm font-black uppercase py-2.5 shadow-classic-md ${
          valid ? 'bg-white dark:bg-black text-accent-primary' : 'bg-white dark:bg-black text-[#8A8A8A] dark:text-[#666] cursor-not-allowed opacity-60'
        }`}
      >
